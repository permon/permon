...
PetscErrorCode QPTHomogenizeEq(QP qp)
{
  ...
  if (is) { PetscCall(VecRestoreSubVector(xtilde, is, &xtilde_sub)); }
...
static PetscErrorCode QPTDualizePostSolve_Private(QP child, QP parent)
{
  ...
  if (alpha) {
    ...
    if (flg) {
      ...
      if (flg) { PetscCall(MatMultTranspose(child->B, child->lambda, child->Bt_lambda)); }
...
PetscErrorCode QPTDualize(QP qp, MatInvType invType, MatRegularizationType regType)
{
  ...
  if (B_view_spectra) { PetscCall(QPTDualizeViewBSpectra_Private(qp->B)); }
  ...
  if (!true_mp) { PetscCall(PetscOptionsGetBool(NULL, NULL, "-qpt_dualize_Kplus_left", &mp, NULL)); }
...
static PetscErrorCode QPTPostSolve_QPTRemoveGluingOfDirichletDofs(QP child, QP parent)
{
  ...
  for (II = 1; II < Mn; II++) { PetscCall(VecCopy(lambda_child[II], lambda_parent[II])); }
...
PetscErrorCode QPTRemoveGluingOfDirichletDofs(QP qp)
{
  ...
  PetscCall(QPTransformBegin(QPTRemoveGluingOfDirichletDofs, QPTPostSolve_QPTRemoveGluingOfDirichletDofs, QPTPostSolveDestroy_QPTRemoveGluingOfDirichletDofs, QP_DUPLICATE_COPY_POINTERS, &qp, &child, &comm));
  ...
  for (II = 1; II < Mn; II++) { PetscCall(QPAddEq(child, mats[II][0], NULL)); }
...
static PetscErrorCode QPTPostSolve_QPTScale(QP child, QP parent)
{
  ...
  PetscFunctionBegin;
  if (ctx->dE) { PetscCall(VecPointwiseMult(parent->lambda_E, ctx->dE, child->lambda_E)); }
  if (ctx->dI) { PetscCall(VecPointwiseMult(parent->lambda_I, ctx->dI, child->lambda_I)); }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode QPTScale(QP qp)
{
  ...
  if (remove_gluing_of_dirichlet) { PetscCall(QPTRemoveGluingOfDirichletDofs(qp)); }

  PetscCall(QPTransformBegin(QPTScale, QPTPostSolve_QPTScale, QPTPostSolveDestroy_QPTScale, QP_DUPLICATE_COPY_POINTERS, &qp, &child, &comm));
  PetscCall(PetscNew(&ctx));
...
PetscErrorCode QPTMatISToBlockDiag(QP qp)
{
  ...
  *  adapted from PCISSetUp */
  ...
  for (i = 0; i < n_neigh; i++) {
    for (j = 0; j < n_shared[i]; j++) { PetscCall(PetscBTSet(bt, shared[i][j])); }
  }
...
PetscErrorCode QPTAllInOne(QP qp, MatInvType invType, PetscBool dual, PetscBool project, PetscReal penalty, PetscBool penalty_direct, PetscBool regularize)
{
  ...
  if (project) { PetscCall(QPTEnforceEqByProjector(qp)); }
...
PetscErrorCode QPTFromOptions(QP qp)
{
  ...
  PetscOptionsBegin(PetscObjectComm((PetscObject)qp), NULL, "QP transforms options", "QP");
  {
    ...
    if (ddm) { invType = MAT_INV_BLOCKDIAG; }
  }
  PetscOptionsEnd();
