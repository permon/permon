...
PetscErrorCode QPFetiAssembleDirichlet(QP qp)
{
  ...
  if (enforce_by_B) {
    ...
    if (flg) {
      ...
      for (i = start; i < stop; i++) { PetscCall(MatSetValue(Bt, dbc_dg_arr[i - start], i, 1.0, INSERT_VALUES)); }
...
PetscErrorCode QPFetiGetBgtSF(MPI_Comm comm, IS i2g, PetscInt Nu, IS i2l, PetscInt nl, FetiGluingType type, Mat *Bgt_out)
{
  ...
  for (i = 0, data_scat_size = 0; i < nroots_SF1; i++) { data_scat_size += root_degree_onroots_SF1[i]; }
    ...
    if (root_degree_onroots_SF1[i] == 1) { //leaves with degree==1 will be deleted (degree==0 dont exist)
      ...
      for (j = 0; j < root_degree_onroots_SF1[i]; j++) {
        ...
        case FETI_GLUING_ORTH:
          ...
          break;
        default:
          SETERRQ(comm, PETSC_ERR_PLIB, "Unknown FETI gluing type");
        }
      }
    }
  }
  ...
  for (i = 0, idx = 0, idx2 = 0; i < nleaves_SF1; i++) {
    for (j = 0; j < future_leavesSF2_onleaves[i]; j++) {
      if (j == 0) { ris_array[idx2] = i2l_array[i]; }
  ...
  for (i = 0, data_scat_size = 0; i < nroots_SF1; i++) { data_scat_size += root_degree_onroots_SF2[i]; }
  ...
  if (type == FETI_GLUING_ORTH) {
    ...
    for (i = 0; i < nleaves_SF2; i++) {
      if (j == 0) {
        ...
        if (root_degree_onleaves_fromSF1onSF2[i] > 2) { j = root_degree_onleaves_fromSF1onSF2[i] - 2; }
      } else if (j >= 1) {
