...
static PetscErrorCode MatOrthColumns_GS_Default(MPI_Comm comm, PetscInt N, Vec q[], Vec s[], PetscScalar dots[], PetscInt *o_max, PetscInt *o_acc)
{
  ...
  for (i = 0; i < N; i++) {
    ...
    do {
      ...
      PetscCall(VecMAXPY(qcur, i, dots, q));
      if (s) { PetscCall(VecMAXPY(s[i], i, dots, s)); }
      PetscCall(VecNorm(qcur, NORM_2, &norm));
      o++;
      o_acc_++;
...
static PetscErrorCode MatOrthColumns_GS(Mat A, MatOrthType type, MatOrthForm form, Mat *Q_new, Mat *S_new)
{
  ...
  if (!f) {
    switch (type) {
    case MAT_ORTH_GS:
      f = MatOrthColumns_GS_Default;
      break;
    case MAT_ORTH_GS_LINGEN:
      f = MatOrthColumns_GS_Lingen;
      break;
    default:
  ...
  if (computeS) {
    ...
    for (i = Slo; i < Shi; i++) { PetscCall(VecSetValue(s[i], i, 1.0, INSERT_VALUES)); }
    for (i = 0; i < N; i++) { PetscCall(VecAssemblyBegin(s[i])); }
    for (i = 0; i < N; i++) { PetscCall(VecAssemblyEnd(s[i])); }
  }
...
PetscErrorCode MatOrthColumns(Mat A, MatOrthType type, MatOrthForm form, Mat *Q_new, Mat *S_new)
{
  ...
  if (!f) {
    ...
    case MAT_ORTH_CHOLESKY:
      f = MatOrthColumns_Cholesky_Default;
      break;
    case MAT_ORTH_IMPLICIT:
    case MAT_ORTH_INEXACT:
      f = MatOrthColumns_Implicit_Default;
      break;
    case MAT_ORTH_NONE:
      PERMON_ASSERT(0, "this should never happen");
    }
  ...
  if (Q_new) { PetscCall(PetscObjectSetName((PetscObject)*Q_new, "Q")); }
  if (S_new) { PetscCall(PetscObjectSetName((PetscObject)*S_new, "S")); }
  PetscFunctionReturnI(PETSC_SUCCESS);
}
