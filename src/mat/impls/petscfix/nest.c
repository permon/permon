...
static PetscErrorCode MatGetColumnVectors_NestPermon(Mat A, Vec *cols_new[])
{
  ...
  CHKERRQ(MatNestGetSubMats(A,&Mn,&Nn,&mats));
  CHKERRQ(PetscMalloc(sizeof(Vec*)*Mn,&cols_for_each_row_block));
  CHKERRQ(PetscMalloc(sizeof(Vec)*N,&cols););
  CHKERRQ(PetscMalloc(sizeof(Vec)*Mn,&col_arr));
  CHKERRQ(PetscMalloc(sizeof(IS)*Nn,&is_glob_col));
  CHKERRQ(MatNestGetISs(A,NULL,is_glob_col));
  ...
  for (J=0; J<Nn; J++) {
    ...
    for (II=0; II<Mn; II++) {
      ...
      CHKERRQ(MatGetColumnVectors(mat,&N1,&cols_for_each_row_block[II]));
      ...
      CHKERRQ(PetscContainerSetPointer(container,cols_for_each_row_block[II]));
      CHKERRQ(PetscObjectCompose((PetscObject)mat,"MatGetColumnVectors_Nest_cols",(PetscObject)container));
    ...
    CHKERRQ(ISAllGather(is_glob_col[J],&is_seq));
    CHKERRQ(ISGetIndices(is_seq,&is_arr));
    
    for (j=0; j<N1; j++) {
      for (II=0; II<Mn; II++) { col_arr[II] = cols_for_each_row_block[II][j]; }
      CHKERRQ(VecCreateNest(comm,Mn,NULL,col_arr,&cols[is_arr[j]]));
    }
    CHKERRQ(ISRestoreIndices(is_seq,&is_arr));
    CHKERRQ(ISDestroy(&is_seq));
  }

  CHKERRQ(PetscFree(is_glob_col));
  CHKERRQ(PetscFree(col_arr));
  CHKERRQ(PetscFree(cols_for_each_row_block));
  *cols_new=cols;
...
static PetscErrorCode MatRestoreColumnVectors_NestPermon(Mat A, Vec *cols[])
{
  ...
  for (II=0; II<Mn; II++) for (J=0; J<Nn; J++) {
    mat = mats[II][J];
    CHKERRQ(PetscObjectQuery((PetscObject)mat,"MatGetColumnVectors_Nest_cols",(PetscObject*)&container));
    CHKERRQ(PetscContainerGetPointer(container,(void**)&block_cols));
    CHKERRQ(MatRestoreColumnVectors(mat,&N1,&block_cols));
    CHKERRQ(PetscObjectCompose((PetscObject)mat,"MatGetColumnVectors_Nest_cols",NULL));
  }
...
static PetscErrorCode MatFilterZeros_NestPermon(Mat A, PetscReal tol, Mat *Af_new)
{
  ...
  MnNn = Mn*Nn;
  CHKERRQ(PetscMalloc(MnNn*sizeof(Mat),&mats_out));
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    CHKERRQ(MatFilterZeros(mats_in[i][j],tol,&mats_out[i*Nn+j]));
  }
  CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)A),Mn,NULL,Nn,NULL,mats_out,Af_new));
  for (i=0; i<MnNn; i++) CHKERRQ(MatDestroy(&mats_out[i]));
...
static PetscErrorCode PermonMatCreateDenseProductMatrix_NestPermon(Mat A, PetscBool A_transpose, Mat B, Mat *C_new)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(PetscObjectTypeCompareAny((PetscObject)B,&B_nest,MATNEST,MATNESTPERMON,""));
  
  if (A_transpose) {
    CHKERRQ(MatNestGetSubMats(A,NULL,&Mn,&A_mats));
  } else {
    CHKERRQ(MatNestGetSubMats(A,&Mn,NULL,&A_mats));
  }

  if (B_nest) {
    CHKERRQ(MatNestGetSubMats(B,NULL,&Nn,&B_mats));
  } else {
    B_p = &B;
    B_mats=&B_p;
    Nn=1;
  ...
  if (Mn==1 && Nn==1) {
    CHKERRQ(PermonMatCreateDenseProductMatrix(A_mats[0][0],A_transpose,B_mats[0][0],C_new));
    PetscFunctionReturn(0);
  }
  ...
  CHKERRQ(PetscMalloc(MnNn*sizeof(Mat),&mats_out));
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    CHKERRQ(PermonMatCreateDenseProductMatrix(A_transpose?A_mats[0][i]:A_mats[i][0],A_transpose,B_mats[0][j],&mats_out[i*Nn+j]));
  }
  CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)A),Mn,NULL,Nn,NULL,mats_out,C_new));
  for (i=0; i<MnNn; i++) CHKERRQ(MatDestroy(&mats_out[i]));
  CHKERRQ(PetscFree(mats_out));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode PermonMatTranspose_NestPermon(Mat A,MatTransposeType type,Mat *At_out)
{
  ...
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    if (mats_in[i][j]) {
      CHKERRQ(PermonMatTranspose(mats_in[i][j],type,&mats_out[j*Mn+i]));
    } else {
      mats_out[j*Mn+i] = NULL;
    }
  }
  CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)A),Nn,NULL,Mn,NULL,mats_out,At_out));
  for (i=0; i<MnNn; i++) CHKERRQ(MatDestroy(&mats_out[i]));
  CHKERRQ(PetscFree(mats_out));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode PermonMatGetLocalMat_NestPermon(Mat A,Mat *Aloc)
{
  ...
  CHKERRQ(PetscMalloc(MnNn*sizeof(Mat),&mats_out));
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    CHKERRQ(PermonMatGetLocalMat(mats_in[i][j],&mats_out[i*Nn+j]));
  }
  CHKERRQ(MatCreateNestPermon(PETSC_COMM_SELF,Mn,NULL,Nn,NULL,mats_out,Aloc));
  for (i=0; i<MnNn; i++) CHKERRQ(MatDestroy(&mats_out[i]));
  CHKERRQ(PetscFree(mats_out));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatMergeAndDestroy_NestPermon(MPI_Comm comm, Mat *local_in, Vec x, Mat *global_out)
{
  ...
  if (x) {
    ...
    CHKERRQ(PetscObjectTypeCompare((PetscObject)x,VECNEST,&flg));
    if (!flg) SETERRQ(comm,PETSC_ERR_ARG_WRONG,"Vec #3 has to be nest");
    CHKERRQ(VecNestGetSubVecs(x,&Nn_cl,&vecs_x));
  ...
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    ...
    CHKERRQ(MatMergeAndDestroy(comm,&data->m[i][j],x_block,&mats_out[i*Nn+j]));
  }
  CHKERRQ(MatCreateNestPermon(comm,Mn,NULL,Nn,NULL,mats_out,global_out));
...
static PetscErrorCode PermonMatConvertBlocks_NestPermon(Mat A, MatType newtype, MatReuse reuse, Mat* B)
{
  ...
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    ...
    CHKERRQ(PermonMatConvertBlocks(block,newtype,reuse,&cblock));
    mats_out[i*Nn+j] = cblock;
  }
  CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)A),Mn,NULL,Nn,NULL,mats_out,&B_));
...
static PetscErrorCode MatNestPermonGetColumnISs_NestPermon(Mat A,IS **is_new)
{
  ...
  CHKERRQ(MatNestGetSubMats(A,&Mn,&Nn,&mats_in));
  CHKERRQ(PetscMalloc1(Nn,&cols));
  CHKERRQ(PetscMalloc1(Nn,&is_jr));
  for (j=0; j<Nn; j++) {
    CHKERRQ(PetscMalloc1(size, &is_jr[j]));
  }
  ...
  for (r=0; r<size; r++) {
    for (j=0; j<Nn; j++) {
      Aij = mats_in[0][j];
      CHKERRQ(MatGetOwnershipRangesColumn(Aij,&ranges_j));
      m = ranges_j[r+1]-ranges_j[r];
      CHKERRQ(ISCreateStride(PETSC_COMM_SELF,m,lo,1,&is_jr[j][r]));
      lo += m;
    }
  }

  for (j=0; j<Nn; j++) {
    CHKERRQ(ISConcatenate(PETSC_COMM_SELF,size,is_jr[j],&cols[j]));
    for (r=0; r<size; r++) {
      CHKERRQ(ISDestroy(&is_jr[j][r]));
    }
    CHKERRQ(PetscFree(is_jr[j]));
  }
  CHKERRQ(PetscFree(is_jr));
...
PetscErrorCode MatNestPermonGetVecs(Mat A,Vec *x,Vec *y)
{
  ...
  CHKERRQ(PetscObjectQueryFunction((PetscObject)A,"MatNestPermonGetVecs_C",&f));
...
static PetscErrorCode MatMatMult_NestPermon_NestPermon(Mat A,Mat B,PetscReal fill,Mat *AB_new)
{
  ...
  PetscFunctionBeginI;
  CHKERRQ(MatNestGetSubMats(A,&M,&K1,&A_mats_in));
  CHKERRQ(MatNestGetSubMats(B,&K2,&N,&B_mats_in));
  PERMON_ASSERT(K1==K2,"# nest columns of A  =  # nest rows of B");

  MN = M*N;
  CHKERRQ(PetscMalloc(K1*sizeof(Mat),&mats_row));
  CHKERRQ(PetscMalloc(MN*sizeof(Mat),&mats_out));
  for (i=0; i<M; i++) {
    for (j=0; j<N; j++) {
      for (k=0; k<K1; k++) {
        if (A_mats_in[i][k]) {
          CHKERRQ(PermonMatMatMult(A_mats_in[i][k],B_mats_in[k][j],MAT_INITIAL_MATRIX,fill,&AB));
        } else {
          AB = NULL;
        }
        mats_row[k] = AB;
      }
      CHKERRQ(MatCreateComposite(PetscObjectComm((PetscObject)A),K1,mats_row,&AB));
      CHKERRQ(MatCompositeSetType(AB,MAT_COMPOSITE_ADDITIVE));
      CHKERRQ(MatCompositeMerge(AB));
      mats_out[i*N+j] = AB;
      for (k=0; k<K1; k++) CHKERRQ(MatDestroy(&mats_row[k]));
    }
  }
  CHKERRQ(PetscFree(mats_row));
  
  if (MN==1) {
    /* handle 1x1 nest as normal matrix */
    AB = *mats_out;
  } else {
    CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)A),M,NULL,N,NULL,mats_out,&AB));
    for (i=0; i<MN; i++) CHKERRQ(MatDestroy(&mats_out[i]));
  }
  CHKERRQ(PetscFree(mats_out));
  *AB_new = AB;
...
static PetscErrorCode MatProductNumeric_NestPermon(Mat C)
{
  ...
  switch (product->type) {
  case MATPRODUCT_AB:
    CHKERRQ(MatMatMult_NestPermon_NestPermon(A,B,product->fill,&newmat));
    break;
...
static PetscErrorCode MatAXPY_NestPermon(Mat Y, PetscScalar a, Mat X, MatStructure str)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(MatNestGetSubMats(Y,&Mn1,&Nn1,&Y_mats_in));
  CHKERRQ(MatNestGetSubMats(X,&Mn,&Nn,&X_mats_in));
  PERMON_ASSERT(Mn=Mn1,"Mn=Mn1");
  PERMON_ASSERT(Nn=Nn1,"Nn=Nn1");
  for (i=0; i<Mn; i++) for (j=0; j<Nn; j++) {
    CHKERRQ(MatAXPY(Y_mats_in[i][j],a,X_mats_in[i][j],str));
  }
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatExtensionCreateCondensedRows_NestPermon(Mat TA,Mat *A,IS *ris_local)
{
  ...
  PetscFunctionBegin;
  CHKERRQ(MatNestGetSubMats(TA,&Mn,&Nn,&mats_in));
  PERMON_ASSERT(Mn==1,"Mn==1");
  CHKERRQ(PetscMalloc1(Nn,&mats_out));
  CHKERRQ(PetscMalloc1(Nn,&ris_block_orig));

  for (j=0; j<Nn; j++) {
    block = mats_in[0][j];
    CHKERRQ(PetscObjectTypeCompare((PetscObject)block,MATEXTENSION,&flg));
    if (!flg) SETERRQ(PetscObjectComm((PetscObject)TA),PETSC_ERR_ARG_WRONG,"nested block %d,0 is not extension",j);
    CHKERRQ(MatExtensionGetRowISLocal(block,&ris_block_orig[j]));
  }

  CHKERRQ(ISConcatenate(PETSC_COMM_SELF,Nn,ris_block_orig,&ris_union));
  CHKERRQ(ISSortRemoveDups(ris_union));
  CHKERRQ(ISGetLocalSize(ris_union,&m));

  for (j=0; j<Nn; j++) {
    block = mats_in[0][j];
    CHKERRQ(ISEmbed(ris_block_orig[j],ris_union,PETSC_TRUE,&ris_block));
    CHKERRQ(MatExtensionGetCondensed(block,&block_A));
    CHKERRQ(MatExtensionGetColumnIS(block,&cis));
    CHKERRQ(MatCreateExtension(PetscObjectComm((PetscObject)TA),m,block->cmap->n,PETSC_DECIDE,block->cmap->N,block_A,ris_block,PETSC_FALSE,cis,&mats_out[j]));
    CHKERRQ(ISDestroy(&ris_block));
  }

  CHKERRQ(MatCreateNestPermon(PetscObjectComm((PetscObject)TA),1,NULL,Nn,NULL,mats_out,A));
  if (ris_local) {
    *ris_local = ris_union;
  } else {
    CHKERRQ(ISDestroy(&ris_union));
  }

  CHKERRQ(PetscFree(mats_out));
  CHKERRQ(PetscFree(ris_block_orig));
  PetscFunctionReturn(0);
}
...
PETSC_EXTERN PetscErrorCode MatConvert_Nest_NestPermon(Mat A,MatType type,MatReuse reuse,Mat *newmat)
{
  ...
  CHKERRQ(PetscObjectChangeTypeName((PetscObject)B,MATNESTPERMON));
  ...
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatGetColumnVectors_C",MatGetColumnVectors_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatRestoreColumnVectors_C",MatRestoreColumnVectors_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatFilterZeros_C",MatFilterZeros_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"PermonMatCreateDenseProductMatrix_C",PermonMatCreateDenseProductMatrix_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"PermonMatTranspose_C",PermonMatTranspose_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"PermonMatGetLocalMat_C",PermonMatGetLocalMat_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatMergeAndDestroy_C",MatMergeAndDestroy_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"PermonMatConvertBlocks_C",PermonMatConvertBlocks_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatNestPermonGetVecs_C",MatNestPermonGetVecs_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatNestPermonGetColumnISs_NestPermon_C",MatNestPermonGetColumnISs_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatNestSetVecType_C",  MatNestSetVecType_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatConvert_nest_nestpermon_C", MatConvert_Nest_NestPermon));
  CHKERRQ(PetscObjectComposeFunction((PetscObject)B,"MatExtensionCreateCondensedRows_Extension_C",MatExtensionCreateCondensedRows_NestPermon));
...
{
  PetscFunctionBegin;
  CHKERRQ(MatSetType(mat,MATNEST));
  CHKERRQ(MatConvert_Nest_NestPermon(mat,MATNESTPERMON,MAT_INPLACE_MATRIX,&mat));
  PetscFunctionReturn(0);
}
...
{
  PetscFunctionBegin;
  CHKERRQ(MatCreateNest(comm,nr,is_row,nc,is_col,a,B));
  CHKERRQ(MatConvert_Nest_NestPermon(*B,MATNESTPERMON,MAT_INPLACE_MATRIX,B));
  PetscFunctionReturn(0);
}
...
static PetscErrorCode MatCreateNestPermonVerticalMerge_Extract_Private(PetscInt nmats_in,Mat *mats_in,PetscInt *nmats_out,Mat **mats_out)
{
  ...
  if (nmats_in == 1) {
    A = mats_in[0];
    CHKERRQ(PetscObjectTypeCompareAny((PetscObject)A,&nest,MATNEST,MATNESTPERMON,""));
    if (nest) {
      CHKERRQ(MatNestGetSize(A,&Mn,NULL));
      if (Mn==1) nest = PETSC_FALSE;
    }
    if (!nest) {
      *nmats_out = 1;
      if (mats_out) {
        CHKERRQ(PetscMalloc1(1,&mats_out_));
        mats_out_[0] = A;
        *mats_out = mats_out_;
      }
    } else {
      CHKERRQ(MatNestGetSubMats(A,&Mn,&Nn,&mats2d));
      if (Nn > 1) SETERRQ(PetscObjectComm((PetscObject)A),PETSC_ERR_ARG_WRONG,"number of nested column blocks exceeds 1");
      CHKERRQ(PetscMalloc1(Mn,&mats));
      for (i=0; i<Mn; i++) mats[i] = mats2d[i][0];
      CHKERRQ(MatCreateNestPermonVerticalMerge_Extract_Private(Mn,mats,nmats_out,mats_out));
  ...
  *nmats_out = 0;
  for (i=0; i<nmats_in; i++) {
    CHKERRQ(MatCreateNestPermonVerticalMerge_Extract_Private(1,&mats_in[i],&nmats_i,NULL));
    *nmats_out += nmats_i;
  }

  if (!mats_out) PetscFunctionReturn(0);
  
  CHKERRQ(PetscMalloc1(*nmats_out,&mats_out_));
  mats_out_p = mats_out_;
  for (i=0; i<nmats_in; i++) {
    CHKERRQ(MatCreateNestPermonVerticalMerge_Extract_Private(1,&mats_in[i],&nmats_i,&mats));
    CHKERRQ(PetscMemcpy(mats_out_p,mats,nmats_i*sizeof(Mat)));
    CHKERRQ(PetscFree(mats));
    mats_out_p += nmats_i;
  }
  *mats_out = mats_out_;
...
PetscErrorCode MatCreateNestPermonVerticalMerge(MPI_Comm comm,PetscInt nmats,Mat mats[],Mat *merged)
{
  ...
  CHKERRQ(MatCreateNestPermonVerticalMerge_Extract_Private(nmats,mats,&nmats_out,&mats_out));
  CHKERRQ(MatCreateNestPermon(comm,nmats_out,NULL,1,NULL,mats_out,merged));
  CHKERRQ(PetscFree(mats_out));
  PetscFunctionReturn(0);
}
