...
static PetscErrorCode MatFilterZeros_NestPermon(Mat A, PetscReal tol, Mat *Af_new)
{
  ...
  for (i = 0; i < Mn; i++)
    for (j = 0; j < Nn; j++) { PetscCall(MatFilterZeros(mats_in[i][j], tol, &mats_out[i * Nn + j])); }
...
static PetscErrorCode PermonMatCreateDenseProductMatrix_NestPermon(Mat A, PetscBool A_transpose, Mat B, Mat *C_new)
{
  ...
  PetscCall(PetscMalloc(MnNn * sizeof(Mat), &mats_out));
  for (i = 0; i < Mn; i++)
    for (j = 0; j < Nn; j++) { PetscCall(PermonMatCreateDenseProductMatrix(A_transpose ? A_mats[0][i] : A_mats[i][0], A_transpose, B_mats[0][j], &mats_out[i * Nn + j])); }
...
static PetscErrorCode PermonMatGetLocalMat_NestPermon(Mat A, Mat *Aloc)
{
  ...
  PetscCall(PetscMalloc(MnNn * sizeof(Mat), &mats_out));
  for (i = 0; i < Mn; i++)
    for (j = 0; j < Nn; j++) { PetscCall(PermonMatGetLocalMat(mats_in[i][j], &mats_out[i * Nn + j])); }
...
static PetscErrorCode MatNestPermonGetColumnISs_NestPermon(Mat A, IS **is_new)
{
  ...
  for (j = 0; j < Nn; j++) { PetscCall(PetscMalloc1(size, &is_jr[j])); }
  ...
  for (j = 0; j < Nn; j++) {
    ...
    for (r = 0; r < size; r++) { PetscCall(ISDestroy(&is_jr[j][r])); }
...
static PetscErrorCode MatAXPY_NestPermon(Mat Y, PetscScalar a, Mat X, MatStructure str)
{
  ...
  for (i = 0; i < Mn; i++)
    for (j = 0; j < Nn; j++) { PetscCall(MatAXPY(Y_mats_in[i][j], a, X_mats_in[i][j], str)); }
  PetscFunctionReturn(PETSC_SUCCESS);
}
