...
static PetscErrorCode MatInvGetKSP_Inv(Mat imat, KSP *ksp)
{
  ...
  PetscFunctionBegin;
  if (!inv->ksp) { PetscCall(MatInvCreateInnerObjects_Inv(imat)); }
  *ksp = inv->innerksp;
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode MatInvSetMat_Inv(Mat imat, Mat A)
{
  ...
  PetscFunctionBegin;
  if (A == inv->A) PetscFunctionReturn(PETSC_SUCCESS);
...
chosen:
  ...
  if (inv->setfromoptionscalled) { PetscCall(KSPSetFromOptions(inv->innerksp)); }
...
PetscErrorCode MatGetInfo_Inv(Mat imat, MatInfoType type, MatInfo *info)
{
  ...
  if (pc && mat->factortype && pc->ops->getfactoredmatrix) {
    ...
    if (mat && mat->ops->getinfo) { PetscCall(MatGetInfo(mat, type, info)); }
  }
...
PetscErrorCode MatView_Inv(Mat imat, PetscViewer viewer)
{
  ...
  if (format == PETSC_VIEWER_ASCII_INFO) {
    ...
    if (inv->innerksp != inv->ksp) {
      ...
      if (show) { PetscCall(KSPViewBriefInfo(inv->innerksp, sv)); }
