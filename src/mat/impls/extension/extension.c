...
static PetscErrorCode MatExtensionSetUp_Extension(Mat TA)
{
  ...
  if (!data->cis) { PetscCall(MatGetOwnershipIS(TA, NULL, &data->cis)); }

  if (data->ris_local && !data->ris) { PetscCall(ISAdd(data->ris_local, lo, &data->ris)); }
  ...
  if (!data->ris_local) { PetscCall(ISAdd(data->ris, -lo, &data->ris_local)); }
...
PetscErrorCode MatConvertFrom_Extension(Mat A, MatType type, MatReuse reuse, Mat *newmat)
{
  ...
  if (!ris) { PetscCall(MatGetOwnershipIS(Aloc, &ris, NULL)); }
  ...
  if (!cis) { PetscCall(MatGetOwnershipIS(Ast, &cis, NULL)); }
...
PETSC_EXTERN PetscErrorCode MatConvert_NestPermon_Extension(Mat A, MatType type, MatReuse reuse, Mat *newmat)
{
  ...
  for (i = 0; i < Mn; i++) {
    ...
    for (j = 0; j < Nn; j++) {
      ...
      if (!risij) { PetscCall(MatGetOwnershipIS(Aloc, &risij, NULL)); }
  ...
  for (i = 0; i < Mn; i++) { PetscCall(ISDestroy(&ris_arr[i])); }
  ...
  for (j = 0; j < Nn; j++) {
    ...
    for (i = 0; i < Mn; i++) {
      ...
      if (!cisij) { PetscCall(MatGetOwnershipIS(Ast, &cisij, NULL)); }
  ...
  for (j = 0; j < Nn; j++) { PetscCall(ISDestroy(&cis_arr[j])); }
  ...
  for (i = 0; i < Mn; i++) {
    for (j = 0; j < Nn; j++) { PetscCall(MatDestroy(&mats_out[i * Nn + j])); }
  }
  ...
  for (j = 0; j < Nn; j++) { PetscCall(ISDestroy(&cols[j])); }
...
PetscErrorCode MatTransposeMatMult_BlockDiag_Extension_2MPIAIJ(Mat B, Mat TA, MatReuse scall, PetscReal fill, Mat *C)
{
  ...
  rnnz_diag = 0;
  for (i = 0; i < rnnz; i++) {
    if (isCols[i] >= nHigh - N_loc && isCols[i] < nHigh) { rnnz_diag += 1; }
  }
...
PetscErrorCode MatMatTransposeMult_Extension_Extension_same(Mat A, Mat B, MatReuse scall, PetscReal fill, PetscInt mattype, Mat *C)
{
  ...
  for (i = 0; i < neighborsLt; i++) { PetscCallMPI(MPI_Irecv(&nnzNeighbors[i], 1, MPIU_INT, iNeighbors[i], 0, comm, &mpiRequests[i])); }
  for (i = neighborsLt + 1; i < allNeighbors; i++) { PetscCallMPI(MPI_Isend(&nnz, 1, MPIU_INT, iNeighbors[i], 0, comm, &mpiRequests[i - 1])); }
  PetscCallMPI(MPI_Waitall(allNeighbors - 1, mpiRequests, MPI_STATUSES_IGNORE));

  /* recv indices from Lt neighbors */
  for (i = 0; i < neighborsLt; i++) { nnzAllLtNeighbors += nnzNeighbors[i]; }
  ...
  for (i = neighborsLt + 1; i < allNeighbors; i++) { PetscCallMPI(MPI_Isend((PetscInt *)iLocCol, nnz, MPIU_INT, iNeighbors[i], 1, comm, &mpiRequests[i - 1])); }
  PetscCallMPI(MPI_Waitall(allNeighbors - 1, mpiRequests, MPI_STATUSES_IGNORE));
  ...
  for (i = 0; i < neighborsLt; i++) {
    ...
    for (j = 0; j < nElemR[i]; j++) {
      k = 0;
      while (iIntersection[j] != iNeighbor[k]) { k += 1; }
      iNeighborRemote[l] = k;
      k                  = 0;
      while (iIntersection[j] != iLocCol[k]) { k += 1; }
      iNeighborInter[l] = k;
      l += 1;
    }
    iNeighbor += nnzNeighbors[i];
  ...
  for (i = 0; i < neighborsLt; i++) { PetscCallMPI(MPI_Isend(data[i], M_loc * nElemR[i], MPIU_SCALAR, iNeighbors[i], 4, comm, &mpiRequests2[i])); }
  ...
  j = 0;
  for (i = neighborsLt + 1; i < allNeighbors; i++) {
    for (k = 0; k < M_loc; k++) { iCol[k] = iNeighbors[i] * M_loc + k; }
    ...
    if (!isSym) { PetscCall(MatSetValues(C_out, M_loc, iCol, M_loc, iRow, arr, INSERT_VALUES)); }
    PetscScalar val;
    for (l = 0; l < M_loc; l++) {
      for (m = 0; m < l; m++) {
        val                = arr[l * M_loc + m];
  ...
  if (myneighbors) { PetscCall(ISRestoreIndices(myneighbors, &iNeighbors)); }
  j = 0;
  for (i = neighborsLt + 1; i < allNeighbors; i++) {
