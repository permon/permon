...
static PetscErrorCode QPSSMALXEGetEta_SMALXE(QPS qps, PetscReal *eta, QPSScalarArgType *argtype)
{
  ...
  PetscFunctionBegin;
  if (eta) { *eta = smalxe->eta_user; }
  if (argtype) { *argtype = smalxe->eta_type; }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode QPSSMALXEGetRhoInitial_SMALXE(QPS qps, PetscReal *rho_initial, QPSScalarArgType *argtype)
{
  ...
  PetscFunctionBegin;
  if (rho_initial) { *rho_initial = smalxe->rho_user; }
  if (argtype) { *argtype = smalxe->rho_type; }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
static PetscErrorCode QPSSMALXEUpdateNormBu_Lag_SMALXEON(QPS qps, Vec u, PetscReal *normBu, PetscReal *enorm)
{
  ...
  if (qps_inner->iteration <= offset) {
    ...
    if (II == 0) {
      ...
      if (rdiff >= upper) {
        PetscCall(PetscInfo(qps, "rdiff = |%.3e / %.3e| = %.3e >= %.3e, ||B*u|| will be recalculated\n", (double)normBu_exact, (double)normBu0, (double)rdiff, (double)upper));
        II = 0;
        J  = Jstart;
      } else if (rdiff < lower) {
        PetscCall(PetscInfo(qps, "rdiff = |%.3e / %.3e| = %.3e < %.3e, ||B*u|| will be recalculated\n", (double)normBu_exact, (double)normBu0, (double)rdiff, (double)lower));
        II = 0;
        J  = Jstart;
      } else {
        II++;
  ...
  if (lag_compare) {
    char sign;
    if (!eval) { PetscCall(QPSSMALXEUpdateNormBu_SMALXEON(qps, u, &normBu_exact, &enorm_exact)); }
...
static PetscErrorCode QPSSMALXEUpdateRho_SMALXE(QPS qps, PetscBool Lagrangian_flag)
{
  ...
  case 1:
    ...
    break;
  case 3:
    rho_update      = smalxe->rho_update_late;
    Lagrangian_flag = PETSC_TRUE;
    break;
  default:
    SETERRQ(PetscObjectComm((PetscObject)qps), PETSC_ERR_ARG_WRONGSTATE, "invalid SMALXE state");
  }
  if (!Lagrangian_flag || rho_update == 1.0) PetscFunctionReturn(PETSC_SUCCESS);

  PetscCall(PetscInfo(qps, "updating rho, multiply by rho_update%" PetscInt_FMT " = %.4e\n", smalxe->state, (double)rho_update));
...
PetscErrorCode QPSSMALXEUpdate_SMALXE(QPS qps, PetscReal Lag_old, PetscReal Lag, PetscReal rho)
{
  ...
  if (smalxe->monitor_outer) {
    ...
    PetscCall(PetscPrintf(comm, "        %.8e %c %.8e  |  %.8e %c %.8e                |  %.8e %c %.8e\n\n", (double)qps->rnorm, (qps->rnorm < cctx->ttol_outer) ? '<' : '>', (double)cctx->ttol_outer, (double)gnorm, (gnorm < qps_inner->atol) ? '<' : '>',
                          (double)qps_inner->atol, (double)gnorm, (gnorm < cctx->gtol) ? '<' : '>', (double)cctx->gtol));
  }
...
PetscErrorCode QPSConverged_Inner_SMALXE(QPS qps_inner, KSPConvergedReason *reason)
{
  ...
  *reason = KSP_CONVERGED_ITERATING;
  ...
  if (smalxe->state == 3 && (i < smalxe->inner_iter_min || smalxe->inner_no_gtol_stop)) { PetscFunctionReturn(PETSC_SUCCESS); }
...
PetscErrorCode QPSSetUp_SMALXE(QPS qps)
{
  ...
  if (smalxe->M1_type == QPS_ARG_MULTIPLE) {
    if (smalxe->maxeig == PETSC_DECIDE) { PetscCall(MatGetMaxEigenvalue(A, NULL, &smalxe->maxeig, smalxe->maxeig_tol, smalxe->maxeig_iter)); }
  ...
  PetscCall(PetscInfo(qps, "   eta=%.8e eta_user=%.8e eta_type=%c\n", (double)smalxe->eta, (double)smalxe->eta_user, smalxe->eta_type == QPS_ARG_DIRECT ? 'D' : 'M'));
  PetscCall(PetscInfo(qps, "maxeig=%.8e\n", (double)smalxe->maxeig));
  PetscCall(PetscInfo(qps, "    M1=%.8e  M1_user=%.8e  M1_type=%c\n", (double)smalxe->M1_initial, (double)smalxe->M1_user, smalxe->M1_type == QPS_ARG_DIRECT ? 'D' : 'M'));
  PetscCall(PetscInfo(qps, "   rho=%.8e rho_user=%.8e rho_type=%c\n", (double)rho, (double)smalxe->rho_user, smalxe->rho_type == QPS_ARG_DIRECT ? 'D' : 'M'));
  ...
  if (!smalxe->inject_maxeig_set) { PetscCall(QPPFGetGHasOrthonormalRows(qp->pf, &smalxe->inject_maxeig)); }
  PetscCall(PetscInfo(qps, "maximum operator eigenvalue estimate %.8e is %sinjected to the inner solver\n", (double)maxeig_inner, smalxe->inject_maxeig ? "" : "NOT "));
...
PetscErrorCode QPSSolve_SMALXE(QPS qps)
{
  ...
  if (smalxe->get_lambda) { PetscCall(QPPFApplyHalfQ(qp->pf, qp->Bt_lambda, qp->lambda)); }
  if (!smalxe->get_Bt_lambda) { PetscCall(VecInvalidate(qp->Bt_lambda)); }
  PetscFunctionReturn(PETSC_SUCCESS);
}
