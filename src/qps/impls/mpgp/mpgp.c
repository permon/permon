...
static PetscErrorCode MPGPExpansionLength(QPS qps)
{
  ...
  case QPS_MPGP_EXPANSION_LENGTH_BB:
    ...
    break;
  default:
    SETERRQ(PetscObjectComm((PetscObject)qps), PETSC_ERR_PLIB, "Unknown MPGP expansion length type");
  }
  PetscFunctionReturn(PETSC_SUCCESS);
}
...
PetscErrorCode QPSSetup_MPGP(QPS qps)
{
  ...
  case QPS_MPGP_EXPANSION_STD:
    ...
    if (mpgp->explengthtype == QPS_MPGP_EXPANSION_LENGTH_FIXED) { mpgp->expproject = PETSC_FALSE; }
    break;
  ...
  case QPS_MPGP_EXPANSION_PROJCG:
    ...
    break;
  default:
    SETERRQ(PetscObjectComm((PetscObject)qps), PETSC_ERR_PLIB, "Unknown MPGP expansion type");
  }
...
PetscErrorCode QPSSolve_MPGP(QPS qps)
{
  ...
  if (mpgp->explengthtype == QPS_MPGP_EXPANSION_LENGTH_BB) {
    ...
    if (mpgp->fallback || mpgp->fallback2) { gold = qps->work[10]; }
    ...
    if (gcTgc <= gamma2 * gfTgf) /* u is proportional */
    {
      ...
      } else                            /* expansion step  */
      {
        ...
        if (mpgp->explengthtype == QPS_MPGP_EXPANSION_LENGTH_BB || mpgp->fallback || mpgp->fallback2) {
          ...
          if (mpgp->explengthtype == QPS_MPGP_EXPANSION_LENGTH_BB) { PetscCall(VecCopy(mpgp->explengthvec, mpgp->explengthvecold)); }
        }

        PetscCall(mpgp->expansion(qps, afeas, acg));
        if (mpgp->expproject) { PetscCall(QPCProject(qpc, x, x)); /* project x to feas.set */ }

        /* compute new gradient */
        if (mpgp->fallback || mpgp->fallback2) { PetscCall(VecCopy(g, gold)); }
        PetscCall(MatMult(A, x, g));    /* g=A*x */
        nmv++;                          /* matrix multiplication counter */
        PetscCall(VecAXPY(g, -1.0, b)); /* g=g-b           */
        ...
        if (mpgp->fallback || mpgp->fallback2) {
          ...
          if (f > fold) {
            ...
            if (mpgp->fallback) {
              ...
              if (mpgp->fallback2) { PetscCall(MPGPGrads(qps, mpgp->xold, gold)); /* grad. splitting  gP,gf,gc */ }
