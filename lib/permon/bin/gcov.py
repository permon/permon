...
PERMON_DIR = os.environ['PERMON_DIR']
...
def run_gcov(gcov_dir):
    ...
    os.mkdir(gcov_dir)
    print("Running gcov\n")
    for root,dirs,files in os.walk(os.path.join(PERMON_DIR,"src")):
        # Directories to skip
        ...
        for file_name in files:
            ...
            if csrc_file:
                ...
                OBJDIR = os.path.join(PERMON_DIR, PETSC_ARCH, 'obj')
                objpath = os.path.join(OBJDIR, os.path.relpath(c_file, os.path.join(PERMON_DIR,"src")))
                gcov_graph_file = objpath+".gcno"
                gcov_data_file  = objpath+".gcda"
                print(gcov_data_file)
                if os.path.isfile(gcov_graph_file) and os.path.isfile(gcov_data_file):
                    ...
                    try:
                        ...
                        root_tmp1 = root.split(PERMON_DIR+os.sep)[1].replace(os.sep,'_')
                    ...
                    file_id.close()
                    lines_fid.close()
    print("""Finshed running gcov on PERMON source code""")
    return

def make_tarball(dirname):

    # Create tarball of .lines files stored in gcov_dir
    print("""Creating tarball in %s to store gcov results files""" %(PERMON_DIR))
    os.chdir(dirname)
    os.system("tar -czf "+PERMON_DIR+os.sep+"gcov.tar.gz *.lines")
    #shutil.rmtree(dirname)
    print("""Tarball created in %s"""%(PERMON_DIR))
    return
...
def make_htmlpage(gcov_dir,LOC,tarballs):
    ...
    #  Stores the main HTML pages in LOC if LOC is defined via command line argument o-wise it uses the default PERMON_DIR
    ...
    for file_ctr in range(0,file_len):
        inhtml_file = PERMON_DIR+os.sep+src_not_tested_path[file_ctr]+os.sep+src_not_tested_filename[file_ctr]+'.html'
    ...
    print("""<html>
    <head>
      <title>PERMON:Code Testing Statistics</title>
    </head>
...
def main():
    ...
    # check to see if LOC is given
        if os.path.isdir(sys.argv[2]):
            ...
            print("No Directory specified for saving main HTML file pages, using PERMON root directory")
            LOC = PERMON_DIR
            tarballs = sys.argv[2:]
