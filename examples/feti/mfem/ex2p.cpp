...
/* PERMON change */
#include <permonksp.h> /* REQUIRED: contains KSPFETI */
#ifdef PERMON_HAVE_MFEM 
...
int main(int argc, char *argv[])
{
   ...
   /* PERMON change: do not save solution by default */
   bool savesol = false;
   /* PERMON change: set refinement levels instead of max number of elements from option */
   int ref_levels = 2;
  ...
  args.AddOption(&ref_levels, "-ref-lvls", "--ref-lvls",
                  "Number of serial mesh refinement levels");
  args.AddOption(&par_ref_levels, "-par-ref-lvls", "--par-ref-lvls",
                 "Number of paralel mesh refinement levels");
  args.AddOption(&savesol, "-save", "--savesol", "-no-save",
                 "--no-savesol",
                 "Enable or disable saving solution into file.");
   args.Parse();
   /* PERMON change: Allow to directly parse PERMON/PETSC options */
   //if (!args.Good())
   //{
   //   if (myid == 0)
   //   {
   ...
   //PermonInitialize(NULL,NULL,petscrc_file,NULL);
   PermonInitialize(&argc,&argv,petscrc_file,NULL); /* PERMON REQUIRED change*/
   ...
   Vector B, X;
   {
      ...
      PetscParMatrix A;
      a->SetOperatorType(Operator::PETSC_MATIS); /* REQUIRED (for FETI): FETI needs Mattype MATIS */
   ...
   /* PERMON change */
   PermonFinalize(); /* REQUIRED: We finalize PERMON */
